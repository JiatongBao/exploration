...
void DualStateFrontier::getUnknowPointcloudInBoundingBox(const StateVec& center, const StateVec& bounding_box_size)
{
  ...
  for (double x_position = bbx_min.x(); x_position <= bbx_max.x(); x_position += kFrontierResolution)
  {
    ...
    {
      //依次遍历给定范围内的x,y,z；调用octomap，根据点坐标得到索引值，并根据索引值得到具体的节点。
      ...
      if (node == NULL)
      {
        //如果octomap中没有这样的节点，就将该点加入到未知点集合unknown_points_中
        unknown_points_->push_back(pcl::PointXYZ(point.x(), point.y(), point.z()));
        //如果该点在给定的范围之内，并且确实是一个边界点（如果该点确实是边界点，那么该点周围应该既有空闲也有未知），那么就加入到local_frontier_temp中
  ...
  //对local_frontier_temp进行滤波得到local_frontier_
...
//给定一个点，检查它是不是边界点
//如果该点x与y方向的紧邻体素均未被占据，且至少有kEffectiveUnknownNumAroundFrontier个体素未知，则判定为边界点
...
void DualStateFrontier::localFrontierUpdate(StateVec& center)
{
  ...
  for (int i = 0; i < local_frontier_->size(); i++)
  {
    ...
    //如果不考虑机器人周围的边界点则过滤掉，配置文件中默认设置的是false，表示要考虑机器人附近的边界点
    //条件1：该边界点与机器人之间没有障碍物（有未知区域可以）
    //或者条件2：非全局规划状态（即探索阶段），在全局图中能找到1个点能够观测到该边界点且之间没有障碍物
    if (!(kEliminateFrontiersAroundRobots && inSensorRangeofRobot(checkedPoint)) && //inSensorRangeofRobot: 距离机器人当前位置在一定阈值之内，并且其向量的俯仰角小于15度，和机器人当前未知进行光线追踪的栅格单元都是free状态，则返回TRUE
        ((manager_->CellStatus::kOccupied != manager_->getVisibility(center, checkedPoint, false) &&
          !grid_->collisionCheckByTerrainWithVector(center, checkedPoint)) ||  //collisionCheckByTerrainWithVector: 给定一个起点和终点，判断二者连线上有没有障碍物
         (!planner_status_ && inSensorRangeofGraphPoints(checkedPoint))))  //inSensorRangeofGraphPoints: 输入一个点，利用kd树，在全局图中找到最近得到点的集合，只要有一个可以被形参位置正常观测，并且没有障碍物，则认为是true
    {
      //将满足一定条件的边界点被加入到local_frontier_pcl_以及global_frontier_中
      local_frontier_pcl_->points.push_back(local_frontier_->points[i]);
      global_frontier_->points.push_back(local_frontier_->points[i]);
    }
  }
  //对local_frontier_pcl_进行降采样点得到local_frontier_
  local_frontier_->clear();
  pcl::VoxelGrid<pcl::PointXYZ> point_ds;
  point_ds.setLeafSize(kFrontierFilterSize, kFrontierFilterSize, kFrontierFilterSize);
...
void DualStateFrontier::globalFrontierUpdate()
{
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    //jtbao: for one-floor indoor environment
    //if(point.z()>1.0)
    //  continue;

    //如果该点属于被清空过的边界点，则跳出本次循环
    if (isCleanedFrontier(global_frontier_->points[i]))
    {
      continue;
    }
    //得到该点在octomap中的索引值，如果该点属于边界点则添加到global_frontier_pcl_中
...
void DualStateFrontier::globalFrontiersNeighbourCheck()
{
  ...
  if (size > 0)
  {
    //遍历 global_frontier_中每一个点，通过KD树搜索其附近的边界点，如果存在，则只保存一个边界点。
    //相当于对全局边界点的一个下采样，并将结果保存到global_frontier_pcl_和global_frontier_中
...
{
  //给定一个搜索范围和位置中心，得到属于边界的点云，并将得到的边界点集合保存到local_frontier_（候选局部边界点集）中
  getUnknowPointcloudInBoundingBox(robot_position_, search_bounding);
  //检查各候选局部边界点，更新局部边界点集local_frontier_，同时保存到候选全局边界点集global_frontier_中
  localFrontierUpdate(robot_position_);
  //检查各候选全局边界点，更新全局边界点集global_frontier_
  globalFrontierUpdate();
  //全局边界点的下采样
