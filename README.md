...
Autonomous exploration in unknown environments has become a critical capability of mobile robots. Many methods often suffer from problems such as exploration goal selection based solely on information gain and inefficient tour optimization. Recent reinforcement learning based methods mainly focus on learning exploration policies and do not consider full area coverage. The performance of transferring learned policy to new environments cannot be guaranteed. To address these issues, a dual-stage exploration method has been proposed, which combines spatial clustering of possible exploration goals and Traveling Salesman Problem (TSP) based tour planning in both local and global scales, aiming for efficient full area exploration in highly convoluted environments. Our method involves two stages: exploration and relocation. During the exploration stage, we introduce to generate local navigation goal candidates straight from clusters of all possible local exploration goals, avoiding the complex calculation of gains for all possible local paths. The local navigation goal is determined through tour planning, utilizing the TSP framework which incorporates both the local navigation goal candidates and the robot's home position. Moreover, during the relocation stage, we suggest clustering all possible global exploration goals and applying TSP-based tour planning to efficiently direct the robot towards previously detected but yet-to-be-explored areas. In addition, a retrying mechanism is introduced to enhance the success rate of exploration. The proposed method is validated in various challenging simulated and real-world environments. Experimental results demonstrate that our method is more effective and efficient in space exploration than compared methods. Videos and code for our method are available at https://github.com/JiatongBao/exploration.
